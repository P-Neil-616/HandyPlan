document.addEventListener("DOMContentLoaded", () => {
    ...
    for (const day of days) {
      for (const item of day.timelineList) {
        if (item.type === "job" && item.job && item.job.isLive) {
          item.job.accumulatedMs = Number(item.job.accumulatedMs || 0);
          item.job.isLive = false;
          item.job.state = "paused";
          pausedAny = true;
        }
  ...
  function pauseTimerIntoDraft() {
    ...
    liveJobRef.timerStartedAt = null;
    saveAll();
...
function recalcSpotsFrom(startIndex) {
  ...
  function renderTimeline() {
    ...
    for (let i = 0; i < d.timelineList.length; i++) {
      ...
      card.onclick = () => {
        ...
        draftJob._active = false; // edited jobs must never become the next fresh draft
  ...
  cancelBtn.onclick = (e) => {
    ...
    const anyLive = days.some(d =>
      d.timelineList.some(it => it.type === "job" && it.job?.isLive)
    );
    ...
    // if we were editing an existing saved job, kill carry-over
    if (editingJobIndex !== null) {
      draftJob = {
        title: "",
        invoiceNumber: null,
  ...
  function openEditorForSpot(spotIndex) {
    ...
    // only create a fresh draft if we are not already holding one
